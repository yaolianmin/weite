...
class admin_lt_job_controller extends adminCommon{
	...
	function ctime_action(){
		...
		if($jobid){
			...
			foreach($rows as $v){
				if($v['status']==2 || $v['edate']<=time()){
					$time=time()+$posttime;
					$id=$this->obj->DB_update_all("lt_job","`edate`='".$time."',`status`='1'","`id`='".$v['id']."'");
				}else{
					$time=$v['edate']+$posttime;
					$id=$this->obj->DB_update_all("lt_job","`edate`='".$time."'","`id`='".$v['id']."'");
				}
			}
