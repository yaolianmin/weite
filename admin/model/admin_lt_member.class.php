...
class admin_lt_member_controller extends adminCommon{
 			...
 			foreach($rows['rows'] as $key=>$val){
				foreach($member as $v){
					if($val['uid']==$v['uid']){
						...
						$rows['rows'][$key]['login_ip']=$v['login_ip'];
					}
				}
				foreach($statis as $v){
					if($val['uid']==$v['uid']){
	...
	function del_action(){
	    ...
	    if($_GET['del']){
	    	$del=$_GET['del'];
	    	if($del){
				$del_array=array("member","lt_info","lt_job","lt_statis","friend_state","question","company_order","attention","answer","answer_review","evaluate_log","invoice_record","coupon_list","friend_state"); 
